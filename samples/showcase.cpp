...
int main(int argc, char** argv) {
    ...
    auto setup = [&app, filename](Engine* engine, View* view, Scene* scene) {
        ...
        app.loader = AssetLoader::create(engine, app.names);
        app.scene = scene;
        if (filename.isEmpty()) {
            return;
        }
        ...
        gltfio::ResourceLoader({engine, assetFolder, true, true}).loadResources(app.asset);
    ...
    auto gui = [&app](filament::Engine* engine, filament::View* view) {
        ...
        if (ImGui::CollapsingHeader("Light")) {
            ImGui::SliderFloat("ibl", &app.iblIntensity, 0.0f, 100000.0f);
